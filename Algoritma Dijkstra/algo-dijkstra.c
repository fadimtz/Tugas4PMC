/* Deskripsi Program digunakan untuk menyimulasikan algoritma dijkstra yang digunakan pada pemetaan daerah.*/
...
int main (){
    ...
    // Input file tanaman
    printf("Selamat datang di database daerah Jawa Barat.\n");
    printf("Silahkan masukkan nama file database nama daerah di Jawa Barat : ");
    ...
    printf("Pembacaan nama daerah di Jawa Barat berhasil.\n");
    printf("Terdapat %d daerah di lokasi yang berbeda\n",n_tanaman);
    // Input graph (berisi jarak masing-masing tanaman). -1 atau 2147483647=INT_MAX menyatakan tak hingga.
    printf("Silahkan masukkan nama file yang berisi jarak masing-masing daerah: ");
    ...
    printf("Pembacaan jarak antar daerah berhasil dilakukan.\n\n");
    //printgraph(n_tanaman,graph_tanaman);
    char progress='y';
    while(progress=='y'){
        // Pembacaan vertex awal dan akhir
        int idx_awal, idx_tujuan;
        printf("\nBerikut adalah daftar daerah yang berada di Jawa Barat.\n");
        for(int i=1;i<=n_tanaman;i++){
            printf("%d. %s\n",i,nama_tanaman[i-1]);
        }
        printf("\nPilih salah satu daerah sebagai posisi awal (Cukup ketik angka saja): ");
        scanf(" %d",&idx_awal);
        idx_awal-=1;
        printf("Pilih salah satu daerah sebagai tujuan (Cukup ketik angka saja): ");
        scanf(" %d",&idx_tujuan);
        idx_tujuan-=1;
        ...
        printf("Apakah anda masih ingin mengelilingi Jawa Barat? [y/n]: ");
        scanf(" %c",&progress);
    }
}